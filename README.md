...
高頻度の株価取得について（重要）
	...
	頻繁な株価取得は、システムへの過負荷になります。
   	「流量制限（秒１０件）」は、注文の設計上の上限です。
   	株価取得や他のコマンドをこの頻度で可能とするキャパシティーは用意していません。
 	
   	高頻度で株価を取得したい場合は、'aCLMMfdsMarketPrice'を利用せず、
	Event機能やWebsocket機能を利用してください。
	
	なおEvent機能やWebsocket機能は、1日10回程度までの接続・切断を想定して作成されています。
	頻繁な切断・再接続は、システムへの過負荷になります。
  	共用のシステムですので過負荷にならないようご利用ください。

	高頻度で株価を取得したい場合は、他社の提供する有料サービスを契約してください。
